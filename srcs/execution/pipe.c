...
/*   Updated: 2026/03/30 21:39:25 by mvignes          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	exec_sec_cmd(t_node *node, t_command *cmd, int pipe[2])
{
	...
	if (pid == 0)
	{
		...
		redir->outfile_fd = what_the_outfile(redir);
		if (redir->outfile_fd == -1)
		{
			close(pipe[0]);
			// free_all(pipex);
			// exit(EXIT_FAILURE);
		}
		redirect_fd(redir->outfile_fd, STDOUT_FILENO);
...
void	exec_first_cmd(t_node *node, t_command *cmd, int pipe[2])
{
	...
	if (pid == 0)
	{
		cmd->infile_fd = open_file_read(cmd->infile);
		close(pipe[0]);
		if (cmd->infile_fd == -1)
		{
			// free_all(pipex);
		...
		redirect_fd(cmd->infile_fd, STDIN_FILENO);
